Feature: US10_gelecegin_bizde_onem_tasir_test
  Background:
    ...
    And   GC Kullanici Gelecegin Bizde Önem Tasir secenegine tiklar
    And   GC Kullanici Gelecegin Bizde Önem Tasir sayfasinda oldugunu dogrular

    Scenario: US10_TC01
      Given GC Kullanici sayfayi kapatir

    Scenario: US10_TC02
      Given GC Kullanici Aras Kargo İs Basvurulari basligini goruntuler
      When  GC Kullanici Aras Kargo İs Basvuruları  altinda aciklamayi goruntuler
      Then  GC Kullanici basvuru seceneklerinin altinda mail adresini goruntuler
      Then  GC Kullanici sayfayi kapatir

    Scenario: US10_TC03
      Given GC Kullanici Aras Kargo İs Basvurulari basligini goruntuler
      When  GC Kullanici Aras Kargo İs Basvurulari  altinda aciklamayi goruntuler
      And   GC Kullanici Linkedin butonuna tiklar
      When  GC Kullanici Aras Kargo Linkedin sayfasinda oldugunu dogrular
      Then  GC Kullanici sayfayi kapatir

    Scenario: US10_TC04
      Given GC Kullanici Aras Kargo İs Başvurulari basligini goruntuler
      When  GC Kullanici Aras Kargo İs Başvurulari  altinda aciklamayi goruntuler
